...
%s
    ...
    static function display_result($error, $data = array(), $lastcode = '', $pagetotal = 0, $html = '') {
        ...
        if (isset($_G['dz_app_errorcode'][$error])) {
            $result['code'] = $_G['dz_app_errorcode'][$error];
        } else {
            $result['code'] = $_G['dz_app_errorcode']['undefined_error'];
        }
        if (isset($_G['dz_app_language'][$error])) {
            $result['msg'] = $_G['dz_app_language'][$error];
        } else {
            $result['msg'] = $_G['dz_app_language']['undefined_error'] . $error;
        }
...
//		$result = self::format_string($result);
     ...
     * */
    function dz_app_get_thumb_image($source, $width, $height = 200, $quality = 75, $border = 0, $type = 2) {
        global $_G;
        $source = trim($source);
        $width = intval($width);
    ...
    static function format_string($params) {
        if (is_array($params)) {
            ...
            if (DZ_APP_CHARSET_OUTPUT != DZ_APP_CHARSET) {
                $params = iconv(DZ_APP_CHARSET, DZ_APP_CHARSET_OUTPUT . "//IGNORE", $params);
            }
    ...
    static function dz_html_entity_decode($string, $replace = 0) {
        if (!$replace) {
