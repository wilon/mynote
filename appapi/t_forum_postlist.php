...
if($_G['forum']['redirect'] || $_G['forum']['type'] == 'group' || empty($_G['forum']['fid'])) {
	showmessage2('Error:板块ID有误！', '', 42001);
}
...
if($_G['forum']['status'] == 3) {
	...
	if($status == -1) {
		showmessage2('Error:forum_not_group', '', 42002);
	} elseif($status == 1) {
		showmessage2('Error:forum_group_status_off', '', 42003);
	} elseif($status == 2) {
		showmessage2('Error:forum_group_noallowed', '', 42004);
	} elseif($status == 3) {
		showmessage2('Error:forum_group_moderated', '', 42005);
	}
...
if($_G['forum']['viewperm'] && !forumperm($_G['forum']['viewperm']) && !$_G['forum']['allowview']) {
	showmessage2('Error:viewperm', '', 42006);
...
if($_G['forum']['password']) {
	if($_G['gp_action'] == 'pwverify') {
		if($_G['gp_pw'] != $_G['forum']['password']) {
			showmessage2('Error:forum_passwd_incorrect', '', 42007);
		} else {
			dsetcookie('fidpw'.$_G['fid'], $_G['gp_pw']);
			showmessage2('Error:forum_passwd_correct', '', 42008);
		}
...
// 得到列表数据
foreach ($_G['forum_threadlist'] as $threadVal) {
    $data[] = array(
            'tid' => $threadVal['tid'],
            'fid' => $threadVal['fid'],
            'author' => $threadVal['author'],
            'authorid' => $threadVal['authorid'],
            'subject' => $threadVal['subject'],
            'dateline' => date("m-d H:i", $threadVal['dateline']),
            'displayorder' => $threadVal['displayorder'],
            'special' => $threadVal['special'],
            'views' => $threadVal['views'],
            'replies' => $threadVal['replies'],
        );
}

if (empty($data)) {
	showmessage2('Error:此版块还没有帖子！', '', 42009);
} else {
