...
if($_G['forum']['status'] == 3) {
	if(!helper_access::check_module('group')) {
		showmessage2('Error:group_status_off', '', 41001);
	}
	require_once libfile('function/group');
	$status = groupperm($_G['forum'], $_G['uid'], 'post');
	if($status == -1) {
		showmessage2('Error:forum_not_group', '', 41002);
	} elseif($status == 1) {
		showmessage2('Error:forum_group_status_off', '', 41003);
	} elseif($status == 2) {
		showmessage2('Error:forum_group_noallowed', '', 41004);
	} elseif($status == 3) {
		showmessage2('Error:forum_group_moderated', '', 41005);
	} elseif($status == 4) {
		if($_G['uid']) {
			showmessage2('Error:forum_group_not_groupmember', '', 41006);
		} else {
			showmessage2('Error:forum_group_not_groupmember_guest', '', 41007);
		}
	} elseif($status == 5) {
		showmessage2('Error:forum_group_moderated', '', 41008);
	}
}

if(empty($_GET['action'])) {
	showmessage2('Error:undefined_action', '', 41009);
} elseif($_GET['action'] == 'albumphoto') {
	require libfile('post/albumphoto', 'include');
} elseif(($_G['forum']['simple'] & 1) || $_G['forum']['redirect']) {
	showmessage2('Error:forum_disablepost', '', 41010);
}
...
if($_GET['action'] == 'edit' || $_GET['action'] == 'reply') {
	...
	if(!empty($thread)) {
		...
		if($thread['readperm'] && $thread['readperm'] > $_G['group']['readaccess'] && !$_G['forum']['ismoderator'] && $thread['authorid'] != $_G['uid']) {
			showmessage2('Error:thread_nopermission', '', 41011);
		}
		...
		showmessage2('Error:thread_nonexistence', '', 41012);
	}

	if($thread['closed'] == 1 && !$_G['forum']['ismoderator']) {
		showmessage2('Error:post_thread_closed', '', 41013);
	}
}
...
if($_G['forum']['password'] && $_G['forum']['password'] != $_G['cookie']['fidpw'.$_G['fid']]) {
	showmessage2('Error:forum_passwd', '', 41014);
}

if(empty($_G['forum']['allowview'])) {
	if(!$_G['forum']['viewperm'] && !$_G['group']['readaccess']) {
		showmessage2('Error:group_nopermission', '', 41015);
	} elseif($_G['forum']['viewperm'] && !forumperm($_G['forum']['viewperm'])) {
		showmessage2('Error:noviewperm', '', 41016);
	}
} elseif($_G['forum']['allowview'] == -1) {
	showmessage2('Error:forum_access_view_disallow', '', 41017);
}
...
if(!$_G['adminid'] && $_G['setting']['newbiespan'] && (!getuserprofile('lastpost') || TIMESTAMP - getuserprofile('lastpost') < $_G['setting']['newbiespan'] * 60) && TIMESTAMP - $_G['member']['regdate'] < $_G['setting']['newbiespan'] * 60) {
	showmessage2('Error:post_newbie_span', '', 41018);
}
...
if(empty($bbcodeoff) && !$_G['group']['allowhidecode'] && !empty($message) && preg_match("/\[hide=?\d*\].*?\[\/hide\]/is", preg_replace("/(\[code\](.+?)\[\/code\])/is", ' ', $message))) {
	showmessage2('Error:post_hide_nopermission', '', 41019);
}
...
if($special == 3 && !isset($_G['setting']['extcredits'][$_G['setting']['creditstrans']])) {
	showmessage2('Error:reward_credits_closed', '', 41020);
}
...
if($_GET['action'] == 'newthread' && $_G['forum']['allowspecialonly'] && !$special) {
	...
	if(!$special && !$specialextra) {
		showmessage2('Error:group_nopermission', '', 41021);
	}
}
...
if($_GET['action'] == 'newthread' || $_GET['action'] == 'newtrade') {
	...
	require_once './source/post/post_newthread.php';
    //require_once libfile('post/post_newthread', 'include');
} elseif($_GET['action'] == 'reply') {
    // 回复主题
	$navtitle .= ' - '.$thread['subject'].' - '.$_G['forum']['name'];
    require_once './source/post/post_newreply.php';
	//require_once libfile('post/post_newreply', 'include');
...
function check_allow_action($action = 'allowpost') {
	...
	if(isset($_G['forum'][$action]) && $_G['forum'][$action] == -1) {
		showmessage2('Error:forum_access_disallow', '', 41022);
	}
}
