...
if($_G['forum']['status'] == 3) {
	...
	if($status == -1) {
		showmessage2('Error:forum_not_group', '', 41001);
	} elseif($status == 1) {
		showmessage2('Error:forum_group_status_off', '', 41002);
	} elseif($status == 2) {
		showmessage2('Error:forum_group_noallowed', '', 41003);
	} elseif($status == 3) {
		showmessage2('Error:forum_group_moderated', '', 41004);
	} elseif($status == 4) {
		if($_G['uid']) {
			showmessage2('Error:forum_group_not_groupmember', '', 41005);
		} else {
			showmessage2('Error:forum_group_not_groupmember_guest', '', 41006);
		}
	} elseif($status == 5) {
		showmessage2('Error:forum_group_moderated', '', 41007);
	}
}

if(empty($_G['gp_action'])) {
	showmessage2('Error:undefined_action', '', 41008);
} elseif($_G['gp_action'] == 'albumphoto') {
	require libfile('post/albumphoto', 'include');
} elseif(($_G['forum']['simple'] & 1) || $_G['forum']['redirect']) {
	showmessage2('Error:forum_disablepost', '', 41009);
}
...
if($_G['gp_action'] == 'edit' || $_G['gp_action'] == 'reply') {
	...
	if($thread = DB::fetch_first("SELECT * FROM ".DB::table('forum_thread')." WHERE tid='$_G[tid]'".($_G['forum_auditstatuson'] ? '' : " AND (displayorder>='0' OR (displayorder IN ('-4', '-2') AND authorid='$_G[uid]'))"))) {
		...
		if($thread['readperm'] && $thread['readperm'] > $_G['group']['readaccess'] && !$_G['forum']['ismoderator'] && $thread['authorid'] != $_G['uid']) {
			showmessage2('Error:thread_nopermission', '', 41010);
		}
		...
		showmessage2('Error:thread_nonexistence', '', 41011);
	}

	if($_G['gp_action'] == 'reply' && ($thread['closed'] == 1) && !$_G['forum']['ismoderator']) {
		showmessage2('Error:post_thread_closed', '', 41012);
	}
}
...
if($_G['forum']['password'] && $_G['forum']['password'] != $_G['cookie']['fidpw'.$_G['fid']]) {
	showmessage2('Error:forum_passwd', '', 41013);
}

if(empty($_G['forum']['allowview'])) {
	if(!$_G['forum']['viewperm'] && !$_G['group']['readaccess']) {
		showmessage2('Error:group_nopermission', '', 41014);
	} elseif($_G['forum']['viewperm'] && !forumperm($_G['forum']['viewperm'])) {
		showmessage2('Error:viewperm', '', 41015);
	}
} elseif($_G['forum']['allowview'] == -1) {
	showmessage2('Error:forum_access_view_disallow', '', 41016);
}
...
if(!$_G['adminid'] && $_G['setting']['newbiespan'] && (!getuserprofile('lastpost') || TIMESTAMP - getuserprofile('lastpost') < $_G['setting']['newbiespan'] * 60) && TIMESTAMP - $_G['member']['regdate'] < $_G['setting']['newbiespan'] * 60) {
	showmessage2('Error:post_newbie_span', '', 41017);
}
...
if(empty($bbcodeoff) && !$_G['group']['allowhidecode'] && !empty($message) && preg_match("/\[hide=?\d*\].*?\[\/hide\]/is", preg_replace("/(\[code\](.+?)\[\/code\])/is", ' ', $message))) {
	showmessage2('Error:post_hide_nopermission', '', 41018);
}
...
if($_G['gp_action'] == 'newthread' && $_G['forum']['allowspecialonly'] && !$special) {
	...
	if(!$special && !$specialextra) {
		showmessage2('Error:group_nopermission', '', 41019);
	}
}
...
function check_allow_action($action = 'allowpost') {
	...
	if(isset($_G['forum'][$action]) && $_G['forum'][$action] == -1) {
		showmessage2('Error:forum_access_disallow', '', 41020);
	}
}
