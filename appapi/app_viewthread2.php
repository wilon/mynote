 ...
 *  展示discuz自带手机页
 *
 *  $ 王伟龙 QQ:973885303 2014-10-20 09:10:14 $
 */
define("IN_MOBILE", 2); // 手机模式
...
if(!$_G['forum_thread'] || !$_G['forum']) {
	showmessage2('thread_nonexistence');
}
...
if(empty($_G['forum']['allowview'])) {
	...
	if(!$_G['forum']['viewperm'] && !$_G['group']['readaccess']) {
		showmessage2('group_nopermission', NULL, array('grouptitle' => $_G['group']['grouptitle']), array('login' => 1));
	} elseif($_G['forum']['viewperm'] && !forumperm($_G['forum']['viewperm'])) {
		showmessage2noperm('viewperm', $_G['fid']);
	}

} elseif($_G['forum']['allowview'] == -1) {
	showmessage2('forum_access_view_disallow');
}
...
if($_G['forum_thread']['readperm'] && $_G['forum_thread']['readperm'] > $_G['group']['readaccess'] && !$_G['forum']['ismoderator'] && $_G['forum_thread']['authorid'] != $_G['uid']) {
	showmessage2('thread_nopermission', NULL, array('readperm' => $_G['forum_thread']['readperm']), array('login' => 1));
}
...
if(empty($_GET['viewpid'])) {
	...
	//include '../data/template/1_diy_touch_forum_viewthread.tpl.php';
	// include template('diy:forum/viewthread'.$sufix.':'.$_G['fid']);
	include './tpl/app_viewthread.php';
} else {
	$_G['setting']['admode'] = 0;
...
function _checkviewgroup() {
	...
	if($status == 1) {
		showmessage2('forum_group_status_off');
	} elseif($status == 2) {
		showmessage2('forum_group_noallowed', 'forum.php?mod=group&fid='.$_G['fid']);
	} elseif($status == 3) {
		showmessage2('forum_group_moderated', 'forum.php?mod=group&fid='.$_G['fid']);
	}
}
